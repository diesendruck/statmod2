...
setwd("~/Google Drive/2. SPRING 2015/STAT MOD 2 - Prof Scott/latent-feature-models")
data <- read.csv("congress109.csv")
data2 <- read.csv("congress109members.csv")

# Helper functions to look at big
peek <- function(M) {
  return (M[1:3,1:3])
}
peekd <- function(M) {
  return (M[(dim(M)[1]-3):dim(M)[1],(dim(M)[2]-3):dim(M)[2]])
}
