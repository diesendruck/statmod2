...
#   }
  ...
  #hyperparams <- c(1, 0.1, 0.000001) # Parameters chosen by Mo.
  inputs <- d$x
  Y <- d$y
  ...
  LML <- function(params) {
    ...
    #v <- LogMarLik(Y, post.c)
    hbs <- PerformLOOCV(d, bandwidth=NULL)
    H <- hbs[[1]]
    sig.sq.hat <- as.matrix(Diagonal(n, x=(t(Y)%*%(diag(n)-H)%*%Y)/(n-2*tr(H)+tr(t(H)%*%H))))
    v <- dmvnorm(Y, sigma=c+sig.sq.hat, log=T)
    return (-v)
  }
  optim(c(5, 0.01), LML)
  
  # Find ideal tuning parameters under marginal likelihood. "GRID" approach.
#   b.range <- seq(0.01, 100, length=50)
#   b.len <- length(b.range)
#   t1.sq.range <- seq(0.01, 100, length=50)
#   t1.sq.len <- length(t1.sq.range)
#   t2.sq <- 0  
#   grid <- matrix(NA, nrow=length(b.range), ncol=length(t1.sq.range))
#   for (i in 1:b.len) {
#     for (j in 1:t1.sq.len) {
#       hyperparams <- c(b.range[i], t1.sq.range[j], t2.sq)
#       c <- Cov(inputs, inputs, hyperparams, Matern52)
#       error.variance <- diag(rep(var(Y), dim(c)[1]))
#       post.c <- c + error.variance
#       #grid[i, j] <- LogMarLik(Y, post.c)
#       grid[i, j] <- dmvnorm(Y, sigma=c+error.variance, log=T)
#     }
#   }
#   colnames(grid) <- seq(0.01, 100, length=50)
#   rownames(grid) <- seq(0.01, 100, length=50)
#   opt.b <- as.numeric(rownames(grid)[which(grid == max(grid), arr.ind = TRUE)[1]])
#   opt.t <- as.numeric(colnames(grid)[which(grid == max(grid), arr.ind = TRUE)[2]])
  ...
  data <- read.csv("weather.csv", header=T, sep=",")
  attach(data)
...
GaussianProcess <- function(inputs, hyperparams, kernel, Y) {
  ...
  if (!is.null(Y)) {
    ...
    for (i in 1:n) {
      ...
      post.mean.i <- Y[i] + k.xs.x%*%ginv(post.c)%*%(Y-mean(Y))
