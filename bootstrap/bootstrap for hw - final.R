...
n <- length(y)
p <- length(betahat) - 1
beta.variance <- sum.squared.errors / (n-p) # Definition of MSE.
xtx <- t(x) %*% x
xtx.inv <- solve(xtx)
betacov <- beta.variance * xtx.inv
beta.std.errs.mse <- as.vector(sqrt(diag(betacov)))

lm1 <- lm(y~x-1)
betacovlm <- vcov(lm1)
beta.std.errs.lm <- as.vector(sqrt(diag(betacovlm)))
# Compare the two estimates of standard error.
cbind(beta.std.errs.mse, beta.std.errs.lm)
